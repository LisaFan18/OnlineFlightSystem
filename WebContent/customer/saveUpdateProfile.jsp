...
<body>
	...
	<%
		String customer_account = (String)request.getParameter("customer_account");
		String customer_email = (String)request.getParameter("customer_email");
	...
	try {
		...
		// check whether the email is used by other customer
	    String checkEmailStr = "SELECT A.acct_num FROM CUSTOMERS C, ACCOUNT A WHERE C.email = A.email AND C.email='" + newEmail + "';";
		ResultSet checkEmailResult = stmt.executeQuery(checkEmailStr);
		if( checkEmailResult.next() && !customer_account.equals(checkEmailResult.getString("acct_num")) ){
			System.out.println("email used by other user!");
			%>
			<script> 
				alert("Sorry, the email you entered has been used by other customer");
				window.location.href = "editProfile.jsp";
			</script>
			<%
			con.close();
		...
		String strQuery = "UPDATE CUSTOMERS C, ACCOUNT A " 
			...
			stm.setString(9, newEmail);
			stm.setString(10, creditCardNo);
			stm.setString(11, customer_email);
			stm.executeUpdate();
			...
			%>
		<script> 
			alert("Congratulation! Update profile success! Please Login in Again!");
		...
		%>
		<script> 
			 alert("Sorry, something went wrong on our server, failed to update profile");
